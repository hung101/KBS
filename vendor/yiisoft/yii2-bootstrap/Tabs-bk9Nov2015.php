...
class Tabs extends Widget
{
    ...
    protected function renderDropdown($itemNumber, &$items, &$panes)
    {
        ...
        foreach ($items as $n => &$item) {
            ...
            // Edward Tan START - 9 Nov 2015 - to fix sub-tab id issue
            $html = Html::tag('div', $content, $options);
            
            if(isset($item['options']['tab_id'])){
                //$options['id'] = $item['options']['tab_id'];
                $item['url'] = '#' . $item['options']['tab_id'];
                // replace auto generated id with fix id
                $html = str_replace($this->options['id'] . '-dd' . $itemNumber . '-tab' . $n, $item['options']['tab_id'], $html);
            }
            
            $panes[] = $html;
            //$panes[] = Html::tag('div', $content, $options);
            
            // Edward Tan END - 9 Nov 2015 - to fix sub-tab id issue
