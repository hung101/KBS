<?php

use yii\helpers\Html;
use yii\widgets\DetailView;

use app\models\general\GeneralLabel;

/* @var $this yii\web\View */
/* @var $model app\models\PenganjuranPemantuan */

$this->title = $model->penganjuran_pemantuan_id;
$this->params['breadcrumbs'][] = ['label' => GeneralLabel::penganjuran_pemantuans, 'url' => ['index']];
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="penganjuran-pemantuan-view">

    <!--<h1><?= Html::encode($this->title) ?></h1>

    <p>
        <?= Html::a('Update', ['update', 'id' => $model->penganjuran_pemantuan_id], ['class' => 'btn btn-primary']) ?>
        <?= Html::a('Delete', ['delete', 'id' => $model->penganjuran_pemantuan_id], [
            'class' => 'btn btn-danger',
            'data' => [
                'confirm' => 'Are you sure you want to delete this item?',
                'method' => 'post',
            ],
        ]) ?>
    </p>
    
    <?= $this->render('_form', [
        'model' => $model,
        'readonly' => $readonly,
    ]) ?>-->

    <?php /*echo DetailView::widget([
        'model' => $model,
        'attributes' => [
            'penganjuran_pemantuan_id',
            'permohonan_pendahuluan_pelagai',
            'menghantar_surat_cuti_tanpa',
            'keperluan_bengkel_telah',
            'membuat_tempahan_penginapan',
            'membuat_tempahan_tempat_untuk',
            'mengesahan_kehadiran_panel',
            'mengesahan_pendaftaran_panel',
            'memberi_taklimat',
            'mengumpul_dan_membukukan',
            'membuat_pelarasan_kewangan',
        ],
    ]);*/ ?>

</div>
