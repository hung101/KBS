...
class BantuanPenyertaanPegawaiTeknikalSearch extends BantuanPenyertaanPegawaiTeknikal
{
    ...
    public function rules()
    {
        return [
            ...
            [['badan_sukan', 'sukan', 'no_pendaftaran', 'alamat_1', 'alamat_2', 'alamat_3', 'alamat_negeri', 
                'alamat_bandar', 'alamat_poskod', 'no_telefon', 'no_faks', 'laman_sesawang', 'facebook', 'twitter', 
                'nama_bank', 'no_akaun', 'nama_kejohanan', 'peringkat', 'peringkat_lain_lain', 'tarikh', 'tempat', 
                'tujuan', 'surat_rasmi_badan_sukan_ms_negeri', 'surat_jemputan_lantikan_daripada_pengelola', 'butiran_perbelanjaan', 
                'salinan_passport', 'maklumat_lain_sokongan', 'status_permohonan', 'catatan', 'tarikh_permohonan', 'jkb', 'tarikh_jkb', 
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            ...
            'tbl_bantuan_penyertaan_pegawai_teknikal.created_by' => $this->created_by,
        ...
        if(Yii::$app->user->identity->sukan){
            ...
            for($i = 0; $i < count($sukan_access); $i++){
                $arr_sukan = null;
                $arr_sukan = array('tbl_bantuan_penyertaan_pegawai_teknikal.sukan'=>$sukan_access[$i]); 
                    array_push($arr_sukan_filter,$arr_sukan);
            }
            
            if(!isset($this->jurulatih)){
                $query->andFilterWhere(['tbl_bantuan_penyertaan_pegawai_teknikal.sukan'=>$arr_sukan_filter]);
            }
        }
